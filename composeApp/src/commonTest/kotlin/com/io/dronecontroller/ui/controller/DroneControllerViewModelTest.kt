...
import com.io.dronecontroller.domain.usecase.*
...
class DroneControllerViewModelTest {
    ...
    @Test
    fun `初期状態は未接続`() = runTest(dispatcher) {
    ...
    @Test
    fun `takeoff成功時にcommandStatusがSuccessになる`() = runTest(dispatcher) {
    ...
    @Test
    fun `takeoff失敗時にcommandStatusがErrorになる`() = runTest(dispatcher) {
    ...
    @Test
    fun `land成功時にcommandStatusがSuccessになる`() = runTest(dispatcher) {
    ...
    @Test
    fun `returnToLaunch成功時にcommandStatusがSuccessになる`() = runTest(dispatcher) {
    ...
    @Test
    fun `toggleMapModeでisMapModeが反転する`() = runTest(dispatcher) {
    ...
    @Test
    fun `startObservingでドローン状態が反映される`() = runTest(dispatcher) {
        vm.startObserving()
