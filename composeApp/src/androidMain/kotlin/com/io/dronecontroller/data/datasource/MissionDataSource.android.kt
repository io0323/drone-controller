...
class MissionDataSource(
    ...
    override suspend fun uploadMission(items: List<DomainMissionItem>): RunStatus<Unit> {
        ...
        return suspendCancellableCoroutine { cont ->
            val disposable =
                drone.mission.uploadMission(plan).subscribe(
    ...
    override suspend fun startMission(): RunStatus<Unit> {
        ...
        return suspendCancellableCoroutine { cont ->
            val disposable =
                drone.mission.startMission().subscribe(
                    { cont.resume(RunStatus.Success(Unit)) },
    ...
    override suspend fun stopMission(): RunStatus<Unit> {
        ...
        return suspendCancellableCoroutine { cont ->
            val disposable =
                drone.mission.clearMission().subscribe(
                    { cont.resume(RunStatus.Success(Unit)) },
    ...
    override suspend fun pauseMission(): RunStatus<Unit> {
        ...
        return suspendCancellableCoroutine { cont ->
            val disposable =
                drone.mission.pauseMission().subscribe(
                    { cont.resume(RunStatus.Success(Unit)) },
    ...
    override fun observeMissionProgress(): Flow<MissionProgress> =
        callbackFlow {
            ...
            val disposable =
                drone.mission.getMissionProgress().subscribe(
                    { progress ->
                        trySend(
                            MissionProgress(
